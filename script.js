...
const forms = state => {
  ...
  const postData = async (url, data) => {
    //асинхронна операція . тому добавили await
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector, closeClickOverlay = true) {
    ...
    trigger.forEach(item => {
      item.addEventListener('click', e => {
        ...
        // document.body.classList.remove('modal-open');  // ще одна можливість відкривання закривання модального вікна за допомоги надаванню класу
      });
    });
...
const tabs = (headerSelector, tabSelector, contentSelector, activeClass, display = 'block') => {
  ...
  function showTabContent(i = 0) {
    content[i].style.display = display; // якщо добавити значення flex . тоді верстка може зїхати
